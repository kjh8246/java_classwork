...
//ArrayList : 순차적(index로 접근)으로 데이터를 관리하는 자료구조입니다. 배열과 유사한 구조입니다.
//HashMap : 순차적인 구조가 아님니다. 인덱스가 없고 key 값을 이용하여  데이터를 접근하여 저장/읽기 한다. 
//			*검색*이 많은 알고리즘에 유용하다.
//위의 2개 클래스는 제너릭 타입을 <> 기호안에 표사한다. HashMap는 K:Key , V:Value 한쌍입니다.
//     제너릭 타입은 int,long,double 기본형 타입 대신에 레퍼(wrapper)클래스로 표기한다.
public class HashMapTest {
	...
	public static void main(String[] args) {
		...
		System.out.println(map);		//toString 제정이되어있습니다. 
		
		//2. 데이터 읽어오기
		System.out.println("Key: 삼겹살의 가격= "+map.get("돼지국밥")+" 원");
		...
		System.out.println(map);	//           ㄴMap은 Key(모든타입)값이 index(정수) 역할을 함
									//			  ㄴKey값은 유일하다?O or 중복가능하다?X	
		map.remove("불고기",10000); 			//인자,리턴형식 : Object -> 모든타입
		//key,value 모두 일치할때 제거가능
