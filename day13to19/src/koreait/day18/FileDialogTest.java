...
// 콘솔에서 입출력으로 실행하는 프로그램 : CLI(Commamd Line Interface) <-> GUI(Graphic user Interface)
// 자바에서도 GUI 를 만들수 있는 클래스들이 있습니다. 그 중 하나인 파일대화상자를 사용합니다.
// Windows OS는 프로그램들이  window라는 틀 안에서 만들어지는 자바에서는 그것을 프레임이라고 합니다.
		...
		fdw.setVisible(true);
		System.out.println("저장한 경로와 파일들 : " + fdw.getDirectory() + fdw.getFile());
