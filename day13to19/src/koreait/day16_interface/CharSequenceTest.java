...
//자바의 CharSequence 라는 인터페이스를 연습합시다. 인터페이스의 활용 예시 입니다.
	...
	public static void main(String[] args) {
		...
		for (int i = 2; i < 5; i++) {
			result += i+"/";			//result 에 연결연산 결과로 다른문자열이 생성되면 주소값이 달라진다.
		...
		for (int i = 2; i < 5; i++) {
			sb.append(i).append(":");	//문자열의 내용이 바뀌어도 객체 자체는 그대로이다.
										//  ㄴ반복되는 연결연산에 효울적입니다.
			System.out.println(sb.hashCode());
		}
		
		//String 클래스 메소드 인자로 CharSequence 타입인 것이 있다.
		//boolean java.lang.String.contains(CharSequence s)
		//			-> 인자의 실제 객체는 다른 클래스틀로 만들어진 것들이 참조될 수 있습니다.
		String semple="2/3/4/5/6/";
		System.out.println(semple.contains(result));	//true
		System.out.println(semple.contains(sb));		//false
