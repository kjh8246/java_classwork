...
public class IntArrayTest4 {
	...
	public static void main(String[] args) {
		...
		//카운트 변수 4개 초기화 -> 분포가 더 다양하나면? 변수가 더 많이 필요하다.->cnt 변수값도 배열로 사용
		int[] cnts = new int[7];
		//위 분포 순서대로 인덱스를 사용하자
		//20개 배열의 값 중에서 70이상의 값을 검색 - 인덱스와 값 출력
		for (int i = 0; i < score.length; i++) {
			//scores[0],scores[1],scores[2],scores[3]...scores[19]
			if (score[i]>=90) cnts[0]++;
			else if (score[i]>=80) cnts[1]++;//90~100
			else if (score[i]>=70) cnts[2]++;
			else if (score[i]>=60) cnts[3]++;
			else if (score[i]>=50) cnts[4]++;
			else if (score[i]>=40) cnts[5]++;
			else cnts[6]++;
